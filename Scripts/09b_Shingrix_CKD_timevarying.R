...
# Date: 29/11/2024
# Version: R 4.3.0
# File name: 09b_Shingrix_CKD_timevarying.R
# Status: Complete
...
# Data sets created: 
## df_CKD
# Description of file: Find CKD status 
# Actions:
...
Shingrix_risktimes <- read_rds(paste0(processed_data, "Shingrix_Risks.rds")) %>%
  select(patid, gender, dob, max_end)
...
df_ckd <- obsfile %>%
  ...
  select(patid, SCr, unit, rangeFrom, rangeTo, obsdate, enterdate, gender, dob, max_end) %>%
...
write_rds(df_ckd[c("patid", "CKD")], paste0(processed_data, "CKD_timevary.rds"))
