...
# Date: 29/11/2024
# Version: R 4.3.0
# File name: 07b_Shingrix_BMI_timevarying.R
# Status: Complete
...
# Data sets created: 
# Description of file: This has been altered from Angel Wong's code (https://github.com/ehr-lshtm/cprd-algorithms/blob/main/CPRD_AURUM/pr_getallbmirecords_Aurum.do) 
# Actions:
## Read in libraries
...
## Find latest measurements, regardless of study entry
...
length(unique(df_BMI2$patid))/length(unique(df_BMI$patid)) #1.1% removed
...
nrow(df_BMI3)/nrow(df_BMI2) #<0.2% removed
...
nrow(df_BMI4)/nrow(df_BMI3) # Removed 0.4%
...
nrow(df_BMI6)/nrow(df_BMI4) # Removed 0.4%
length(unique(df_BMI6$patid))/length(unique(df_BMI4$patid))
  
  # Remove patient df_BMI if it is later than max exitdate (This has changed from the original script to allow for updated BMI)
  df_BMI6  <- df_BMI6  %>%
    filter(max_end >= obsdate)  
...
##################### Find patient's whose BMI is latest #######################
  ...
  write_rds(df_long[c("patid", "BMI")], paste0(processed_data, "BMI_timevary.rds" ))
